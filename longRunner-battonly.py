...
runs the runOptimiser(battonly) for every day within a specified period -> outputs daily results to CSV
...
Sources *every variable* from its CSVs, the carbonintensity API and the Mixergy API
Calculates APX and SSP costs for later analysis in Excel """
# T: optimises battery only, not mixergy tanks
...
elecgen_date = datetime.datetime(2021, 1, 1)

# T: How many days should the longrunner run for? (the second input number)
# For 24hr period
elecgen_dates = [[strfdelta(elecgen_date, delta)] for delta in range(0, 3)]
...
with open('./longRun/fin-CPoptimCoT7V2.csv', 'w', newline='') as csvfile:
    ...
    for run_date in elecgen_dates:
        ...
        # Call the optimizer
        optimized = runOptimizerBattonly.main(new_conf)
        ...
        # T: oioioioioioioioi +/--, check your signs, Tonda
        # T: fixed hopefully
