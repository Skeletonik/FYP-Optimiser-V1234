...
def validate(system:object) -> object:
    # Make sure the energy availability parameters are of consistent length
...
def constructProblem(system:object) -> pulp.LpProblem:
...
def solveLp (prob:pulp.LpProblem) -> object:
...
def main(system:object) -> object:
    ...
    if validation['status'] != 0:
        return validation
    # CONSTRUCT
    prob = constructProblem(system)
    # SOLVE
    feedback = solveLp(prob)
    # RETURN
    return feedback
